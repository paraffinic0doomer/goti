-- Goti hardening for the safety features.
--
-- SEPARATE MIGRATION ON PURPOSE. The previous migration ADDS the enum values
-- UNDER_REVIEW / POT / POT_CONTRIBUTION; PostgreSQL refuses to let a new enum
-- value be USED in the same transaction that created it (SQLSTATE 55P04).
-- Splitting lets the enum additions commit first — the same expand-contract
-- discipline DATABASE.md prescribes for columns.

-- ===== Hardening for the safety features (source: prisma/sql/hardening-safety.sql) =====
-- ============================================================================
