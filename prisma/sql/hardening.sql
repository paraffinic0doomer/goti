...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_transaction_events_unpublished
...
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_wallets_needs_attention
  ON wallets (status, updated_at)
  WHERE status <> 'ACTIVE';

-- The money-request expiry sweep. Never scans settled requests.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_money_requests_expiring
  ON money_requests (expires_at)
  WHERE status = 'REQUESTED';

-- Money-request notifications still to be delivered.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_money_requests_unnotified
  ON money_requests (updated_at)
  WHERE notified_at IS NULL;

-- The analyst review queue.
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_risk_flags_open
...
-- DATABASE.md "The CREATE INDEX CONCURRENTLY problem".
